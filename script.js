...
var MOCK_USERS = [
  {
    ...
    purpose: "Internship",
    company: "Deloitte New York",
    role: "Associate Consultant",
    location: "Frankfurt, Germany",
    field: "Consulting",
    bio: "Former Kolping resident, now building a career in management consulting. Always happy to chat about NYC life and career paths in consulting.",
    online: true
  },
  {
    id: 2,
    ...
    purpose: "University Exchange",
    company: "KPMG",
    role: "Audit Junior",
    location: "Cologne, Germany",
    field: "Finance",
    bio: "Studied at NYU Stern during my time at Kolping. Now doing audit at KPMG Cologne. Big fan of the Kolping community.",
    online: true
  },
  {
    id: 3,
    ...
    location: "London, UK",
    field: "Finance",
    bio: "Interned at a startup in Brooklyn while at Kolping. Ended up in IB. NYC shaped who I am professionally and personally.",
    online: false
  },
  {
    id: 4,
    ...
    field: "Technology",
    bio: "Went to NYC to improve my English, left with lifelong friends and a clearer sense of direction. Working in product now.",
    online: false
  },
  {
    id: 5,
    ...
    field: "Consulting",
    bio: "Interned in finance in Midtown while living at Kolping. Now at McKinsey Berlin. Happy to connect with fellow alumni!",
    online: true
  },
  {
    id: 6,
    ...
    role: "VP M&A Advisory",
    location: "New York, USA",
    field: "Finance",
    bio: "Loved NYC so much I never really left. Now in M&A. The Kolping years were foundational - happy to mentor younger alumni.",
    online: true
  },
  {
    id: 7,
    ...
    field: "Design",
    bio: "Just returned from NYC. Did my design internship at a creative agency in SoHo while living at Kolping. Looking to connect!",
    online: true
  }
];
...
var DEFAULT_EVENTS = [
  {
    id: 1,
    title: "Cologne Alumni Meetup",
    date: "May 10, 2025",
    location: "Cologne, Germany",
    attending: 14,
    emoji: "🍻",
    desc: "Casual get-together for Kolping NYC alumni living in or around Cologne. Bar TBD.",
    rsvp: false
  },
  {
    id: 2,
    title: "NYC Reunion 2025",
    date: "June 21, 2025",
    location: "New York, USA",
    attending: 31,
    emoji: "🗽",
    desc: "Annual reunion at the Kolping House NYC. Open to all alumni from all years.",
    rsvp: false
  },
  {
    id: 3,
    ...
    attending: 8,
    emoji: "☕",
    desc: "Monthly casual online call. Share what you're up to, ask questions, reconnect.",
    rsvp: false
  },
  {
    id: 4,
    title: "Frankfurt Finance Lunch",
    date: "May 22, 2025",
    location: "Frankfurt, Germany",
    attending: 6,
    emoji: "📊",
    desc: "For Kolping alumni working in finance in Frankfurt. Networking lunch.",
    rsvp: false
  }
];
...
function saveState() {
  try {
    localStorage.setItem("kolping_state_v2", JSON.stringify(STATE));
  } catch (e) {}
}
...
function renderNetwork(container) {
  ...
  var info = div("");
  var nameEl = div("my-card-name");
  nameEl.appendChild(txt(ME.name));
  var roleEl = div("my-card-role");
  roleEl.appendChild(txt(ME.role + " · " + ME.company));
  var locEl = div("my-card-loc");
  locEl.appendChild(txt(ME.location));
  ...
  var s1 = div("");
  s1.innerHTML = '<div class="my-stat-val">' + STATE.connections.length + '</div><div class="my-stat-lbl">connections</div>';

  var cohortCount = 0;
  for (var ci = 0; ci < MOCK_USERS.length; ci++) {
    if (overlapYears(MOCK_USERS[ci].years, ME.years)) cohortCount++;
  }

  var s2 = div("");
  s2.innerHTML = '<div class="my-stat-val">' + cohortCount + '</div><div class="my-stat-lbl">cohort peers</div>';
  ...
  var cl = div("cohort-label");
  cl.appendChild(txt("Your Kolping Cohort"));
  ch.appendChild(cl);
  ...
  cohortUsers.forEach(function (u) {
    ...
    var name = div("cohort-avatar-name");
    name.appendChild(txt(u.name.split(" ")[0]));
  ...
  var yfTitle = div("section-title");
  yfTitle.appendChild(txt("Filter by year"));
  ...
  if (!STATE.yearFilter) {
    var sameField = MOCK_USERS.filter(function (u) {
      return u.field === ME.field;
    });
    appendSuggestedSection(pad, "Same field as you", "Finance professionals", sameField);

    var sameCity = MOCK_USERS.filter(function (u) {
      return u.location === ME.location;
    });
    appendSuggestedSection(pad, "In your city", ME.location, sameCity);
  }

  var filtered = STATE.yearFilter
    ? MOCK_USERS.filter(function (u) {
        return u.years.from <= STATE.yearFilter && u.years.to >= STATE.yearFilter;
      })
    : MOCK_USERS;

  var listTitle = div("section-title");
  listTitle.innerHTML =
    (STATE.yearFilter ? "Class of " + STATE.yearFilter : "All Members") +
  ...
  filtered.forEach(function (u) {
    list.appendChild(makeMemberCard(u));
  });
...
function appendSuggestedSection(container, title, subtitle, users) {
  ...
  var t = div("suggested-section-title");
  t.appendChild(txt(title));
  var s = div("suggested-section-sub");
  s.appendChild(txt(subtitle));
  hdr.appendChild(t);
  hdr.appendChild(s);
  sec.appendChild(hdr);
  ...
  users.forEach(function (u) {
    ...
    var n = div("suggested-card-name");
    n.appendChild(txt(u.name));
    var r = div("suggested-card-role");
    r.appendChild(txt(u.role));
    var tag = div("tag-year");
    tag.appendChild(txt(u.years.from + "–" + u.years.to));
...
function makeMemberCard(user) {
  ...
  var name = div("member-name");
  name.appendChild(txt(user.name));
  var role = div("member-role");
  role.appendChild(txt(user.role));

  var tags = div("member-tags");
  var ty = div("tag-year");
  ty.appendChild(txt(user.years.from + "–" + user.years.to));
  var tl = div("tag-loc");
  tl.appendChild(txt("📍 " + user.location.split(",")[0]));
  tags.appendChild(ty);
  tags.appendChild(tl);
...
function openProfile(userId) {
  ...
  var nameBlock = div("");
  var pName = div("profile-name");
  pName.appendChild(txt(user.name));
  var pRole = div("profile-role");
  pRole.appendChild(txt(user.role + " at " + user.company));
  var pLoc = div("profile-loc");
  pLoc.appendChild(txt("📍 " + user.location));

  nameBlock.appendChild(pName);
  nameBlock.appendChild(pRole);
  nameBlock.appendChild(pLoc);
  topRow.appendChild(nameBlock);

  if (user.online) {
    var ob = div("profile-online-badge");
    ob.appendChild(txt("● Online"));
    topRow.appendChild(ob);
  }
  body.appendChild(topRow);

  var kb = div("profile-kolping-badge");
  var kbIcon = div("pkb-icon");
  kbIcon.appendChild(txt("🏠"));
  var kbText = div("");
  var kbLabel = div("pkb-label");
  kbLabel.appendChild(txt("Kolping House " + user.years.from + "–" + user.years.to));
  var kbSub = div("pkb-sub");
  kbSub.appendChild(txt(user.purpose + " · New York City"));
  kbText.appendChild(kbLabel);
  kbText.appendChild(kbSub);
  kb.appendChild(kbIcon);
  kb.appendChild(kbText);
  body.appendChild(kb);

  var aboutTitle = div("profile-about-title");
  aboutTitle.appendChild(txt("About"));
  var bio = div("profile-bio");
  bio.appendChild(txt(user.bio));
...
function renderMessages(container) {
  var pad = div("tab-pad");
  var title = div("page-title");
  title.appendChild(txt("Messages"));
  var sub = div("page-sub");
  sub.appendChild(txt(STATE.connections.length + " connections"));
  pad.appendChild(title);
  pad.appendChild(sub);

  var chatIds = Object.keys(STATE.messages).map(Number);
  if (chatIds.length) {
    var rTitle = div("section-title");
    rTitle.appendChild(txt("Recent"));
  ...
  var cTitle = div("section-title");
  cTitle.appendChild(txt("Your Connections"));
  pad.appendChild(cTitle);

  var cList = div("chat-list");
  var connUsers = MOCK_USERS.filter(function (u) {
    return isConnected(u.id);
  });

  if (!connUsers.length) {
    var empty = div("empty-state");
    empty.appendChild(txt("Connect with alumni to start chatting"));
    cList.appendChild(empty);
  } else {
    connUsers.forEach(function (u) {
      cList.appendChild(makeChatRow(u, u.location, null, true));
    });
  }
...
function makeChatRow(user, preview, time, isConn) {
  ...
  var n = div("chat-row-name");
  n.appendChild(txt(user.name));
  var p = div("chat-row-last");
  p.appendChild(txt(preview));
  info.appendChild(n);
  info.appendChild(p);
  row.appendChild(info);

  if (time) {
    var t = div("chat-row-time");
    t.appendChild(txt(time));
    row.appendChild(t);
  } else if (isConn) {
    var m = div("tag-msg");
    m.appendChild(txt("Message"));
    row.appendChild(m);
  }
...
function openChat(userId) {
  ...
  var nameBlock = div("");
  var nbName = el("div", "");
  nbName.style.cssText = "font-weight:700;font-size:15px;color:#1A1A1A;";
  nbName.appendChild(txt(user.name));

  var nbOnline = el("div", "");
  nbOnline.style.cssText = "font-size:11px;color:" + (user.online ? "#4CAF50" : "#AAA") + ";";
  nbOnline.appendChild(txt(user.online ? "● Online" : "Offline"));

  nameBlock.appendChild(nbName);
  nameBlock.appendChild(nbOnline);
...
function renderChatMessages(userId) {
  ...
  msgs.forEach(function (msg) {
    ...
    var textNode = div("");
    textNode.appendChild(txt(msg.text));
    var timeEl = div("bubble-time");
    timeEl.appendChild(txt(msg.time));
...
function renderFeed(container) {
  var pad = div("tab-pad");
  var title = div("page-title");
  title.appendChild(txt("Activity"));
  var sub = div("page-sub");
  sub.appendChild(txt("Updates from your network"));
  pad.appendChild(title);
  pad.appendChild(sub);
  ...
  MOCK_FEED.forEach(function (item) {
    ...
    var iconEl = div("feed-icon");
    iconEl.appendChild(txt(item.icon));
    ...
    strong.appendChild(txt(nameStr));
    feedText.appendChild(strong);
    feedText.appendChild(txt(rest));

    var feedTime = div("feed-time");
    feedTime.appendChild(txt(item.time));

    body.appendChild(feedText);
    body.appendChild(feedTime);
  ...
  var ctaIcon = div("feed-cta-icon");
  ctaIcon.appendChild(txt("📣"));
  var ctaTitle = div("feed-cta-title");
  ctaTitle.appendChild(txt("Share an update"));
  var ctaSub = div("feed-cta-sub");
  ctaSub.appendChild(txt("Let your network know what's new"));
  var postBtn = el("button", "btn-post-update");
  postBtn.appendChild(txt("Post update"));
  postBtn.addEventListener("click", function () {
    alert("Post updates coming soon!");
  });
...
function renderEvents(container) {
  var pad = div("tab-pad");
  var title = div("page-title");
  title.appendChild(txt("Events"));
  var sub = div("page-sub");
  sub.appendChild(txt("Alumni meetups & reunions"));
  pad.appendChild(title);
  pad.appendChild(sub);
  ...
  STATE.events.forEach(function (event) {
    ...
    var emojiEl = div("event-emoji");
    emojiEl.appendChild(txt(event.emoji));

    var headerText = div("");
    var eTitle = div("event-title");
    eTitle.appendChild(txt(event.title));
    var eMeta = div("event-meta");
    eMeta.appendChild(txt(event.date + " · " + event.location));

    headerText.appendChild(eTitle);
    headerText.appendChild(eMeta);
    header.appendChild(emojiEl);
    header.appendChild(headerText);
    card.appendChild(header);

    var body = div("event-body");
    var desc = div("event-desc");
    desc.appendChild(txt(event.desc));
    var footer = div("event-footer");

    var attEl = div("event-attending");
    ...
    footer.appendChild(attEl);
    footer.appendChild(rsvpBtn);
...
function renderOpportunities(container) {
  ...
  var titleBlock = div("");
  var oTitle = div("page-title");
  oTitle.appendChild(txt("Opportunities"));
  var oSub = div("page-sub");
  oSub.style.marginBottom = "0";
  oSub.appendChild(txt("Jobs, referrals & requests"));
  titleBlock.appendChild(oTitle);
  titleBlock.appendChild(oSub);
  topRow.appendChild(titleBlock);

  var postBtn = el("button", "btn-post-opp");
  ...
  if (STATE.showOppForm) {
    ...
    var formTitle = div("opp-form-title");
    formTitle.appendChild(txt("New Post"));
    ...
    var pubBtn = el("button", "btn-publish");
    pubBtn.appendChild(txt("Publish"));
  ...
  var filtered =
    STATE.oppFilter === "All"
      ? STATE.opportunities
      : STATE.opportunities.filter(function (o) {
          return o.field === STATE.oppFilter;
        });

  var list = div("opp-list");
  if (!filtered.length) {
    var empty = div("empty-state");
    empty.appendChild(txt("No posts in this category yet."));
    ...
    filtered.forEach(function (opp) {
      ...
      var avWrap2 = div("");
      avWrap2.style.cursor = "pointer";
      avWrap2.appendChild(makeAvatar(user, 38));
      (function (uid) {
        avWrap2.addEventListener("click", function () {
          openProfile(uid);
        });
      })(user.id);
      ...
      var un = div("opp-card-name");
      un.appendChild(txt(user.name));
      var ur = div("opp-card-role");
      ur.appendChild(txt(user.role));
      userInfo.appendChild(un);
      userInfo.appendChild(ur);
      cardTop.appendChild(userInfo);

      var badge = div("badge-" + opp.type);
      badge.appendChild(txt(TYPE_LABELS[opp.type] || opp.type));
      cardTop.appendChild(badge);
      card.appendChild(cardTop);

      var cardTitle = div("opp-card-title");
      cardTitle.appendChild(txt(opp.title));
      card.appendChild(cardTitle);

      var footer2 = div("opp-card-footer");
      var fieldTag = div("tag-field-gold");
      fieldTag.appendChild(txt(opp.field));
      var timeEl = div("opp-card-time");
      timeEl.appendChild(txt(opp.time));
      footer2.appendChild(fieldTag);
      footer2.appendChild(timeEl);
